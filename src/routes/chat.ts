...
router.use(createCustomModelMiddleware("chat"));
...
router.post("/", (req: Request, res: Response) => {
  ...
  const provider = chatProviders[modelEntry.owned_by];
  if (!provider) {
    res.status(400).json({
  ...
  req.on("close", () => {
    if (!res.writableEnded) {
      console.warn(`[chat] [${modelId}] client disconnected before response completed`);
    }
  });

  // Catch write errors (e.g. EPIPE on streaming responses)
  res.on("error", (err: Error) => {
    console.error(`[chat] [${modelId}] response write error: ${err.message}`);
  });
  ...
  Promise.resolve()
    ...
    .catch((err: Error) => {
      console.error(`[chat] [${modelId}] unhandled provider error: ${err.message}`, err.stack);
      if (!res.headersSent) {
